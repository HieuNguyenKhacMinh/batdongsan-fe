...
@Injectable()
export class  BalconyDirectionService {
  url = 'http://localhost:3000/cms/balcony-direction';
  propertiesUrl = ''
  ...
  getProperties() {
    let headers = { authorization: localStorage.getItem('authorization') || '', site_id: localStorage.getItem('site_id') || '' }
    return this.http.get(`http://localhost:3000/cms/table-config/balcony_direction`, { headers });
  }
  update(data: any) {
    let headers = {
