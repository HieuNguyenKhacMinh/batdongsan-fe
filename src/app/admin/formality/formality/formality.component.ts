...
export class FormalityComponent implements OnInit {
  ...
  constructor(private formalityService: FormalityService, public dialog: MatDialog) {

  }

  ngOnInit(): void {
    //get table properties
    this.formalityService.getProperties().subscribe((res: any) => {
  ...
  getDatasource() {
 // set datasource
 this.formalityService.all().subscribe((res: any) => {
  this.dataSource = res;
})
  }
  ...
  openDialog(dataSource?: any): void {
    const dialogRef = this.dialog.open(CreateFormalityComponent, {
      width: '550px',
  ...
  confirmDialog(dataSource?: any): void {
    const dialogRef = this.dialog.open(DeleteFormalityComponent, {
      width: '550px',
      data: {properties: this.properties, dataSource}
    });
